...
<template>
  <div class="declareDecision">
    ...
    <popup v-if="popupshow" :type="popuptype" @cancel="cancel" @rankNum = "rankNum" :rank = "ranking"></popup>
    <DialogCommon
      v-if="detailFlag"
      type="detail"
  ...
  methods: {
    ...
    getState(name, row) {
      ...
      if (name == "通过") {
        ...
        this.currentpreDeclareId = row.companyCode;
      } else if (name == "挂起") {
        this.popupshow = true;
        ...
        detailBasicinfotopic(row.approvalId).then(res => {
          if (res.result == "SUCCESS") {
    ...
    cancel(obj) {
      if (obj == "") {
        ...
        //   通过
        approvePreDeclareApproval({
          id: this.itemRow.approvalId, // 审批id
          topicId: this.itemRow.topicId, // 主题ID
          rank: obj.ranking,
          preDeclareId: this.itemRow.id, // 预申报id
          comments: obj.content, // 排名意见
          companyCode: this.itemRow.companyCode // 组织机构代码
        }).then(res => {
          if (res.result == "SUCCESS") {
            this.popupshow = false;
            this.getInformationData(
        ...
        failPreDeclareApproval({
          id: this.currentid,
          preDeclareId: this.currentpreDeclareId,
        ...
        // 挂起
        hangupPreDeclareApproval({
          id: this.currentrow.approvalId,
          preDeclareId: this.currentrow.id,
          approvalComment: obj.content
        }).then(res => {
          this.popupshow = false;
          this.getInformationData(
    ...
    approveFn() {
      ...
      if (!this.multipleSelection.length) {
        ...
        if (flag) {
          approvedBasicinfotopic(arr).then(res => {
            if (res.result == "SUCCESS") {
              this.$message({
                message: "批量审批通过已成功!",
                type: "success"
