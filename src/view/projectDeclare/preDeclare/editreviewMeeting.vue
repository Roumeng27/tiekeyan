		...
		async getReviewmeetingInfo(){
			...
			searchReviewmeetingInfo(this.$route.query.id).then((res)=>{
				...
				if(res.result == 'SUCCESS'){
					...
					this.responsibleAccount = res.data.responsibleAccount;// 责任人
					this.responsibleCompany = res.data.responsibleCompany;// 责任单位
					...
					this.company = arrWork; 
					// this.company = [res.data.responsibleCompany];
