<template>
  <!-- 修改申报信息 -->
  <div class="pageWrapper" v-if="show">
    ...
    <div class="wrapperSearch">
      ...
      <div class="editItem" style="margin-top:20px;">
        <div class="editCenter">
          <div class="centerArea">
            ...
            <div class="editNameselect">
              <!-- choicedatas -->
              <selectTree
                :choicedatas="proposerAccountarrs"
                @choicedata="proposerAccountarr"
                :number="1"
                :choicelevel="3"
              ></selectTree>
            </div>
        ...
        <div class="editCenter">
          ...
          <div class="editNameselect">
            <selectTree
              :choicedatas="proposerCompanyCodearrs"
              @choicedata="proposerCompanyCodearr"
              :number="1"
              :endlevel="0"
            ></selectTree>
          </div>
      ...
      <div class="editItem" style="margin-top:20px;">
        <div class="editCenter">
          <div class="centerArea">
            ...
            <div class="editNameselect">
              <selectTree
                :choicedatas="presenterCompanyCodearrs"
                @choicedata="presenterCompanyCodearr"
                :number="1"
                :endlevel="0"
              ></selectTree>
            </div>
        ...
        <div class="editCenter">
          ...
          <div class="editNameselect">
            <selectTree @choicedata="companyCodesarr" :choicedatas="companyCodesarrs" :endlevel="0"></selectTree>
          </div>
        </div>
      </div>
      <div class="editItem" style="margin-top:20px;">
        <div class="editCenter">
          <div class="centerArea">
            ...
            <div class="editNameselect">
              <selectTree
                :choicedatas="responsibleNamearrs"
                @choicedata="responsibleNamearr"
                :number="1"
								:choicelevel="3"
              ></selectTree>
            </div>
    ...
    <div class="wrapperTextarea">
      <div class="editArea">
        <p>
          <span class="star">*</span>项目背景及技术经济分析
        </p>
...
import { searchBasicinfotopic, editBasicinfotopic } from "http/projectDeclare";
export default {
  name: "editbasicInfo",
  components: {
    topTitle,
  ...
  data() {
    return {
      ...
      proposerAccountarrs: [],
      // 建议单位
      proposerCompanyCodearrs: [],
      // 负责人
      responsibleNamearrs: [],
      // 主持单位
      presenterCompanyCodearrs: [],
      // 参与单位
      companyCodesarrs: []
    };
  },
  created() {
    this.getbasicIndoDetail(); // 查看项目基本信息 详情
  },
  methods: {
    // 查看项目基本信息 详情
    getbasicIndoDetail() {
      ...
      console.log(res);
      //   searchBasicinfotopic(this.$route.query.id).then(res => {
      //     if (res.result == "SUCCESS") {
      this.preDeclareNo = res.preDeclareNo; // 项目预申报编号
      this.name = res.name; // 项目名称
      this.subject = res.subject; // 所属专业
      this.type = res.type; // 项目类别
      this.monthNum = res.monthNum; // 实施周期
      this.startDate = res.startDate; // 项目开始日期
      this.endDate = res.endDate; // 项目结束日期
      // this.proposerAccount = res.proposerName; // 建议人
      // this.proposerCompanyCode = res.pCompanyName; // 建议单位
      // this.responsibleName = res.responsibleName; // 项目责任人
      this.contactPhone = res.contactPhone; // 联系电话
      this.budget = res.budget; // 总预算
      this.selfRaisedFund = res.selfRaisedFund; // 自筹资金
      this.appropriationFunds = res.appropriationFunds; // 拨款资金
      this.background = res.background; // 项目背景及技术经济分析
      this.mainStudyContent = res.mainStudyContent; // 主要研究内容
      this.objectives = res.objectives; // 研究目标
      this.expect = res.expect; // 预期成果
      this.intellectualProperty = res.intellectualProperty; // 知识产权目标
      this.remark = res.remark; // 备注
      // this.presenterCompanyCode = res.presenterName; // 主持单位
      // this.companyCodes = res.otherNames; // 参与单位
			// 建议人
			if(res.proposerAccount &&res.proposerName){
				// console.log(1)
				this.proposerAccountarrs = [
					{
						code: res.proposerAccount,
						name: res.proposerName
					}
				];
				this.proposerAccount = this.proposerAccountarrs;
			}
			// console.log(this.proposerAccount)
      
			// 建议单位
			if(res.proposerCompanyCode&&res.pCompanyName){
				this.proposerCompanyCodearrs = [
					{
						code: res.proposerCompanyCode,
						name: res.pCompanyName
					}
				];
				this.proposerCompanyCode = this.proposerCompanyCodearrs;
			}
			// 负责人
			if(res.responsibleAccount&&res.responsibleName){
				this.responsibleNamearrs = [
					{
						code: res.responsibleAccount,
						name: res.responsibleName
					}
				];
				this.responsibleName = this.responsibleNamearrs;
			}
			// 主持单位
			if(res.presenterCompanyCode&&res.presenterName){
				this.presenterCompanyCodearrs = [
					{
						code: res.presenterCompanyCode,
						name: res.presenterName
					}
				];
				this.presenterCompanyCode = this.presenterCompanyCodearrs;
			}
			
      res.preDeclareCompanyVoList.map((v, i) => {
        var arr = {
					name : v.voName,
					code : v.voCompanyCode 
				}
				this.companyCodesarrs.push(arr)
			});
			this.companyCodes = this.companyCodesarrs
      // 参与单位
      // this.companyCodesarrs;
      //   }
      // });
    },
    //建议人
    ...
    saveBasicInfo() {
      if (!this.background) {
        ...
        editBasicinfotopic({
          ...
          id: this.$route.query.id.id, // 主题id
