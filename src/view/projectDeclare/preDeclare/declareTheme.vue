<template>
  <div class="declareTheme">
    ...
    <div class="addtheme">
      <router-link to="/preDeclare/declareTheme/addTheme" tag="button" class="searchbtn">创建主题</router-link>
    </div>
    ...
    <DialogCommon
      ...
      :msg="dialogMsg"
      type="success"
      @successUnselect="successUnselect"
    ></DialogCommon>
  ...
  data() {
    return {
      ...
      operation: {
        ...
        // 关闭 CLOSE
        data5: [{ name: "详情", icon: "baI_details" }]
      },
      loading: false,
      // 分页器所需字段
      // 总条数
        ...
        if (res.result == "SUCCESS") {
          ...
          if (res.data.list != null && res.data.list.length > 0) {
            res.data.list.map((v, i) => {
              if (
                v.departmentCode == this.loginInforma.departmentCode &&
                v.companyCode == this.loginInforma.companyCode
              ) {
                v.Hierarchy = "本层主题";
              } else {
                v.Hierarchy = "上层主题";
    ...
    getState(name, row) {
      if (name == "关闭") {
        if (row.status == "RELEASE") {
          ...
          this.dialogMsg =
            "主题关闭后,将无法访问和操作主题,子主题下的相关内容，是否确定关闭?";
        } else {
          this.errorFlag = true;
